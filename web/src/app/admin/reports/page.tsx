...
const STATUS_LABELS: Record<string, { label: string; icon: typeof CheckCircle; color: string }> = {
  ...
  resolved: { label: '已处理', icon: CheckCircle, color: 'bg-green-100 text-green-700' },
  dismissed: { label: '已驳回', icon: XCircle, color: 'bg-gray-100 text-gray-500' },
};
    ...
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">审核处理用户举报内容</p>
        </div>
      ...
      <div className="grid grid-cols-3 gap-4 mb-6">
        {[
          { label: '待处理', count: reports.filter(r => r.status === 'pending').length, color: 'text-yellow-600 bg-yellow-50' },
          { label: '已处理', count: reports.filter(r => r.status === 'resolved').length, color: 'text-green-600 bg-green-50' },
          { label: '已驳回', count: reports.filter(r => r.status === 'dismissed').length, color: 'text-gray-600 bg-gray-50' },
        ].map((card) => (
        ...
        <table className="w-full">
          <thead>
            <tr className="border-b border-gray-200 bg-gray-50">
              <th className="text-left px-4 py-3 text-xs font-medium text-gray-500 uppercase">举报人</th>
              ...
              filtered.map((report) => {
                ...
                return (
                  <tr key={report.id} className="border-b border-gray-100 hover:bg-gray-50">
                    ...
                    <td className="px-4 py-3 text-sm text-gray-600 max-w-xs truncate" title={report.reason}>
                      {report.reason}
                    </td>
                        ...
                        {report.status === 'pending' ? (
                          <>
                            ...
                            <button
                              ...
                              className="text-xs px-2.5 py-1 rounded bg-gray-100 text-gray-600 hover:bg-gray-200"
                            >
