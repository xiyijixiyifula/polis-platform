    ...
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">管理平台所有帖子和内容</p>
        </div>
        ...
        <table className="w-full">
          <thead>
            <tr className="border-b border-gray-200 bg-gray-50">
              ...
              <th className="text-center px-4 py-3 text-xs font-medium text-gray-500 uppercase">模块</th>
              <th className="text-center px-4 py-3 text-xs font-medium text-gray-500 uppercase">精选</th>
              <th className="text-right px-4 py-3 text-xs font-medium text-gray-500 uppercase">浏览</th>
              ...
              <tr key={post.id} className="border-b border-gray-100 hover:bg-gray-50">
                <td className="px-4 py-3">
                  <div className="flex items-center gap-2">
                    <span className={`text-sm font-medium ${post.is_deleted ? 'text-gray-400 line-through' : 'text-gray-900'}`}>
                      {post.title?.slice(0, 40) || '(无标题)'}
                    </span>
                  </div>
                </td>
                <td className="px-4 py-3 text-center">
                  <span className="text-xs bg-gray-100 text-gray-600 px-2 py-0.5 rounded">{post.module_type}</span>
                </td>
                <td className="px-4 py-3 text-center">
                  {post.is_featured ? (
                ...
                <td className="px-4 py-3 text-right text-sm text-gray-500">{formatCount(post.view_count)}</td>
                <td className="px-4 py-3 text-right text-sm text-gray-500">{formatCount(post.like_count)}</td>
