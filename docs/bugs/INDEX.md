...
| 已归类 Pattern | 17 |
| 回归链数 | 12 |
| 总复发次数 | 12（URL编码 ×3 + xattr ×2 + Array.map ×1 + Gateway路由 ×1 + 部署流程 ×1 + 模块标签硬编码 ×2 + localStorage残留 ×1 + URL参数缺失 ×1 + sqlx列不匹配 ×1） |
| 复发率 | 12.4%（12/97） |
| 修复配方数 | 18 |
| 修复点位地图 | [fix-points.md](fix-points.md) |
| 最近更新 | 2026-06-08 (v0.3.x 模块发帖引用 + localStorage残留 + sqlx列不匹配) |
