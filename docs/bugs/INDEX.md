...
| 已归类 Pattern | 18 |
| 回归链数 | 14 |
| 总复发次数 | 14（URL编码 ×3 + xattr ×2 + Array.map ×1 + Gateway路由 ×1 + 部署流程 ×1 + 模块标签硬编码 ×2 + localStorage残留 ×1 + URL参数缺失 ×1 + sqlx列不匹配 ×1 + internal-id-leak ×3 + CSP ×1） |
| 复发率 | 13.9%（14/101） |
| 修复配方数 | 19 |
| 修复点位地图 | [fix-points.md](fix-points.md) |
| 最近更新 | 2026-06-09 (探索页命名 + PostCard链接 + 钱包CLI + CSP回归) |
